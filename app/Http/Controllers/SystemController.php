...
class ManagerMenuData {
    ...
    public static function jsonMenu() {
        $menus = Menu::where('depth', 1)->where('status', 1)->where('belong_to', 'manager')->get()->sortByDesc('sort');
        $submenus = Menu::where('depth', 2)->where('status', 1)->where('belong_to', 'manager')->get()->sortByDesc('sort');
